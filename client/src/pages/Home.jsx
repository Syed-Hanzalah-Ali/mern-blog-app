import React, { useEffect, useState } from 'react'
import {Link} from "react-router-dom"
import AddCallToAction from '../components/AddCallToAction'
import PostCard from '../components/PostCard'
export default function Home() {

  const [posts,setPosts]=useState([])

  useEffect(()=>{
    async function fetchPosts(){
      const response=await fetch('/api/v1/posts/getposts')
      const result=await response.json()
      if(result.success===true){
        // console.log(result.data);
        setPosts(result.data.posts)
        
      }
    }
    fetchPosts()
  },[])
  // console.log(posts);
  
  return (
    <div>
      <div className='flex flex-col gap-3 px-3 p-28 max-w-6xl mx-auto'>
        <h1 className='text-3xl font-bold lg:text-6xl '>Welcome to my Blog</h1>
        <p className='text-gray-500 text-sm sm:text-sm'>Here you will find articles related to computer science.</p>
        <Link to={'/search'}
        className='text-xs sm:text-sm text-teal-500 font-bold hover:underline'>View all posts</Link>
      </div>

      <div className='p-3 bg-amber-100 dark:bg-slate-700'>
        <AddCallToAction/>
      </div>

      <div className='max-w-6xl mx-auto p-3 flex flex-col gap-8 py-7'>
        {
          (posts&&posts.length>0)&&(
            <div className='flex flex-col gap-6'>
              <h2 className='text-2xl font-semibold text-center'>Recent Posts</h2>

              <div className='flex flex-wrap gap-4'>
                {posts.map((post)=>{
                  return(
                    <PostCard key={post._id} post={post}/>
                  )
                })}
              </div>
              <Link to={'/search'} className='text-lg text-teal-500 hover:underline text-center'>View all posts</Link>
            </div>
          )
        }
      </div>

    </div>
  )
}
